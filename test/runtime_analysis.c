...
#include "matrix_reordering.h"  // o el archivo de encabezado correcto para matrixReordering
...
extern void triangularMatrixVectorMultCUDA(int m, double *a, int *ia, int *ja, double *x, double *b);
int main(int argc, char **argv)
{
    ...
    double start1, end1, start2, end2, start3, end3, start4, end4, start5, end5;
    double elapsed_time, best_time1, best_time2, best_time3, best_time4, best_time5;
    double total_time1 = 0, total_time2 = 0, total_time3 = 0, total_time4 = 0, total_time5 = 0;
    double *b_copy, norm_2, norm_2_MKL;
...
/* --- Parallel Solution of Sparse Triangular Linear System --- */
        ...
        for(i = 0; i <= iter_count; i++)
        {
            ...
            if(i == 0)
				triangularMatrixVectorMultCUDA(m, a, ia, ja, x_real, b);
            else
            {
				...
				triangularMatrixVectorMultCUDA(m, a, ia, ja, x_real, b);
				end5 = omp_get_wtime();
				elapsed_time = end5 - start5;
                total_time5 += elapsed_time;
                best_time5 = (elapsed_time < best_time5) ? elapsed_time : best_time5;
            }
        }
...
printf("MKL Seq. Triangular Solver \t| Min. runtime: %.4f ms \t| Avg. runtime: %.4f ms\n" \
       ...
       "CUDA Sparse Triangular solver. \t| Min. runtime: %.4f ms \t| Avg. runtime: %.4f ms\n", \
       best_time1 * 1000, total_time1 , \
       best_time2 * 1000, total_time2 , \
       best_time3 * 1000, total_time3 , \
       best_time4 * 1000, total_time4 , \
       best_time5 * 1000, total_time5 );
